alter table public.household_reminders
...
begin
  ...
  elsif tg_op = 'UPDATE' and new.due_at is distinct from old.due_at and new.status = 'pending' then
    update public.internal_notifications
    set read_at = null, body = new.message, created_at = now()
...
after insert or update of status, due_at on public.household_reminders
