...
const moreDestinationPaths = new Set([
  "/contacts",
  "/documents",
  "/finance",
  "/household",
          ...
          {navigationItems.map((item) => {
            const isCurrent =
              item.href === "/"
                ? pathname === "/"
                : pathname === item.href ||
                  (item.href === "/more" && moreDestinationPaths.has(pathname));
            return (
              <li key={item.label}>
                <Link className={styles.navigationLink} href={item.href} aria-current={isCurrent ? "page" : undefined}>
