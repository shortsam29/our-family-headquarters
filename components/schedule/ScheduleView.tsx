...
function EventCard({ event, members, canManage }: { event: ScheduleEvent; members: ManagedFamilyMember[]; canManage: boolean }) {
  return <article className={styles.event} data-category={event.category}>
    <div><h3>{event.title}</h3><p><time dateTime={event.date}>{event.allDay ? "All day" : `${event.startTime ?? ""}${event.endTime ? `â€“${event.endTime}` : ""}`}</time>{event.location ? ` Â· ${event.location}` : ""}</p></div>
...
export default function ScheduleView({ events, members, canManage, today }: { events:ScheduleEvent[]; members:ManagedFamilyMember[]; canManage:boolean; today:string }) {
  ...
  return <div className={styles.calendar}>
    <div className={styles.toolbar}><div className={styles.views}>{(["day","week","month","agenda"] as const).map(v=><button key={v} aria-pressed={view===v} onClick={()=>setView(v)}>{v[0].toUpperCase()+v.slice(1)}</button>)}</div><div className={styles.navigation}><button onClick={()=>move(-1)} aria-label="Previous period">â€¹</button><button onClick={()=>setSelected(today)}>Today</button><input aria-label="Choose date" type="date" value={selected} onChange={e=>setSelected(e.target.value)}/><button onClick={()=>move(1)} aria-label="Next period">â€º</button></div></div>
