...
const categoryClasses: Record<ScheduleEvent["category"], string> = {
  household: styles.categoryHousehold,
  family: styles.categoryFamily,
  school: styles.categorySchool,
  work: styles.categoryWork,
  appointment: styles.categoryAppointment,
  celebration: styles.categoryCelebration,
};

function EventForm({ event, events, members, selectedDate, onSaved, onCancel }: { event?: ScheduleEvent; events: ScheduleEvent[]; members: ManagedFamilyMember[]; selectedDate?: string; onSaved?: () => void; onCancel?: () => void }) {
  const router = useRouter();
  const formRef = useRef<HTMLFormElement>(null);
  const [eventDate, setEventDate] = useState(event?.seriesStartDate ?? event?.date ?? selectedDate ?? "");
  const [startTime, setStartTime] = useState(event?.startTime ?? "");
  const [endTime, setEndTime] = useState(event?.endTime ?? "");
  const [allDay, setAllDay] = useState(event?.allDay ?? false);
  const [state, action, pending] = useActionState(saveScheduleEventState.bind(null, event?.seriesId ?? event?.id ?? null), { status: "idle" } as ScheduleActionState);
  const conflicts = useMemo(() => {
    if (!eventDate) return [];
    return [...new Set(events.filter((candidate) => {
      if (candidate.seriesId === event?.seriesId || candidate.date !== eventDate) return false;
      if (allDay || candidate.allDay) return true;
      const candidateStart = candidate.startTime ?? "00:00";
      const candidateEnd = candidate.endTime ?? candidateStart;
      const proposedEnd = endTime || startTime;
      return Boolean(startTime && proposedEnd > candidateStart && startTime < candidateEnd);
    }).map((candidate) => candidate.title))];
  }, [allDay, endTime, event?.seriesId, eventDate, events, startTime]);
  useEffect(() => {
  ...
  return <form ref={formRef} action={action} className={styles.form}>
    ...
    <label>Category<select name="category" defaultValue={event?.category ?? "household"}><option value="household">Household</option><option value="family">Family</option><option value="school">School</option><option value="work">Work</option><option value="appointment">Appointment</option><option value="celebration">Celebration</option></select></label>
    <label>{event?.recurrence ? "Series start date" : "Date"}<input name="date" type="date" value={eventDate} onChange={(change) => setEventDate(change.target.value)} required /></label>
    <label className={styles.check}><input name="allDay" type="checkbox" checked={allDay} onChange={(change) => setAllDay(change.target.checked)} />All day</label>
    <div className={styles.timeGroup}><label>Start<input name="startTime" type="time" value={startTime} onChange={(change) => setStartTime(change.target.value)} /></label><label>End<input name="endTime" type="time" value={endTime} onChange={(change) => setEndTime(change.target.value)} /></label></div>
    <label>Location<input name="location" defaultValue={event?.location} maxLength={240} /></label>
    <div className={styles.timeGroup}><label>Repeat<select name="recurrence" defaultValue={event?.recurrence ?? ""}><option value="">Does not repeat</option><option>daily</option><option>weekly</option><option>monthly</option><option>yearly</option></select></label><label>Reminder<select name="reminderMinutes" defaultValue={event?.reminderMinutes ?? ""}><option value="">No reminder</option><option value="15">15 minutes before</option><option value="60">1 hour before</option><option value="1440">1 day before</option></select></label></div>
    ...
    {conflicts.length ? <p className={styles.warning} role="status">Scheduling note: this overlaps {conflicts.join(", ")}. You can still save it if the timing is intentional.</p> : null}
...
function EventCard({ event, events, members, canManage, onSaved }: { event: ScheduleEvent; events: ScheduleEvent[]; members: ManagedFamilyMember[]; canManage: boolean; onSaved: () => void }) {
  const [editing, setEditing] = useState(false);
  return <article className={`${styles.event} ${categoryClasses[event.category]}`}><div><h3>{event.title}</h3><p><time dateTime={event.date}>{event.allDay ? "All day" : `${displayTime(event.startTime)}${event.endTime ? ` - ${displayTime(event.endTime)}` : ""}`}</time>{event.location ? ` - ${event.location}` : ""}</p>{event.recurrence ? <span className={styles.repeatBadge}>Repeats {event.recurrence}</span> : null}</div>{event.description ? <p>{event.description}</p> : null}{canManage ? <div><button type="button" className={styles.editButton} onClick={() => setEditing((value) => !value)}>{editing ? "Close editor" : "Edit event"}</button>{editing ? <><EventForm event={event} events={events} members={members} onSaved={() => { setEditing(false); onSaved(); }} onCancel={() => setEditing(false)} /><form action={deleteScheduleEvent.bind(null, event.seriesId ?? event.id)}><button className={styles.danger}>{event.recurrence ? "Delete series" : "Delete event"}</button></form></> : null}</div> : null}</article>;
}
...
export default function ScheduleView({ events, members, canManage, today, initialDate }: { events: ScheduleEvent[]; members: ManagedFamilyMember[]; canManage: boolean; today: string; initialDate?: string }) {
  ...
  return <div className={styles.calendar}>
    ...
    <div className={styles.legend} aria-label="Calendar color key">{Object.entries(categoryClasses).map(([category, className]) => <span key={category} className={className}>{category}</span>)}</div>
    {adding ? <EventForm events={events} members={members} selectedDate={selected} onSaved={() => { setAdding(false); setNotice("Event saved. It is now on the household calendar."); }} onCancel={() => setAdding(false)} /> : null}
    {view === "month" ? <><div className={styles.weekdays} aria-hidden="true">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => <span key={day}>{day}</span>)}</div><div className={styles.monthGrid}>{dates.map((date) => { const dayEvents = events.filter((event) => event.date === date); const outside = new Date(`${date}T12:00:00`).getMonth() !== selectedMonth; return <button type="button" key={date} className={`${styles.monthDay} ${date === today ? styles.currentDay : ""} ${date === selected ? styles.selectedDay : ""} ${outside ? styles.outsideMonth : ""}`} onClick={() => setSelected(date)} aria-label={`${label(date, { month: "long", day: "numeric" })}, ${dayEvents.length} events`}><time dateTime={date}>{Number(date.slice(-2))}</time>{dayEvents.slice(0, 3).map((event) => <span className={categoryClasses[event.category]} key={event.id}>{event.title}</span>)}{dayEvents.length > 3 ? <small>+{dayEvents.length - 3} more</small> : null}</button>; })}</div><section className={styles.selectedEvents}><h3>Events for {label(selected, { weekday: "long", month: "long", day: "numeric" })}</h3>{selectedEvents.length ? selectedEvents.map((event) => <EventCard key={event.id} event={event} events={events} members={members} canManage={canManage} onSaved={() => setNotice("Event saved. It is now on the household calendar.")} />) : <p>No events scheduled for this day.</p>}</section></> : view === "week" ? <div className={styles.weekGrid}>{dates.map((date) => <section key={date} className={`${styles.day} ${date === today ? styles.currentDay : ""}`}><button className={styles.dayHeading} onClick={() => { setSelected(date); setView("day"); }}><time dateTime={date}>{label(date, { weekday: "short", month: "short", day: "numeric" })}</time></button>{events.filter((event) => event.date === date).map((event) => <EventCard key={event.id} event={event} events={events} members={members} canManage={canManage} onSaved={() => setNotice("Event saved. It is now on the household calendar.")} />)}</section>)}</div> : <div className={styles.timeline}>{["All day", "Morning", "Afternoon", "Evening"].map((part) => <section key={part}><h3>{part}</h3><div>{selectedEvents.filter((event) => daypart(event) === part).map((event) => <EventCard key={event.id} event={event} events={events} members={members} canManage={canManage} onSaved={() => setNotice("Event saved. It is now on the household calendar.")} />)}{!selectedEvents.some((event) => daypart(event) === part) ? <p>No events</p> : null}</div></section>)}</div>}
  </div>;
}
