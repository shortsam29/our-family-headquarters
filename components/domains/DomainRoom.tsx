...
function ExtraFields({ slug, shoppingType }: { slug: DomainSlug; shoppingType?: "grocery" | "household" }) {
  ...
  if (slug === "shopping") return <><input type="hidden" name="listType" value={shoppingType ?? "household"}/><label>List name<input name="listName" placeholder={shoppingType === "grocery" ? "Groceries" : "Shopping"} maxLength={100} /></label><label>Quantity<input name="quantity" maxLength={40} /></label><label>Category or aisle<input name="category" maxLength={80} /></label><label>Priority<select name="priority" defaultValue="normal"><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option></select></label></>;
          ...
          {records.map((record) => (
            <li key={record.id}>
              <Card className={styles.record}>
                ...
                {slug === "shopping" ? (
                  <form action={toggleShoppingItem.bind(null, record.id, record.status !== "purchased", shoppingType === "grocery" ? "/grocery" : "/shopping")}>
                ...
                {(canManage || slug === "shopping") ? (
                  <details>
                    ...
                    <form action={updateDomainRecord.bind(null, slug, record.id)} className={styles.editForm}>
                      ...
                      {canManage ? <button className={styles.danger} formAction={removeDomainRecord.bind(null, slug, record.id, slug === "shopping" && shoppingType === "grocery" ? "/grocery" : undefined)} type="submit">Remove</button> : null}
                    </form>
                  </details>
                ) : null}
              </Card>
