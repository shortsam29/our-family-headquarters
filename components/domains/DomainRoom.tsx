...
const copy: Record<DomainSlug, { title: string; add: string; empty: string }> = {
  meals: { title: "Meal", add: "Plan a meal", empty: "No meals are planned yet. A simple week can begin whenever youâ€™re ready." },
...
function ExtraFields({ slug, shoppingType }: { slug: DomainSlug; shoppingType?: "grocery" | "household" }) {
  ...
  if (slug === "shopping") return <><input type="hidden" name="listType" value={shoppingType ?? "household"}/><label>Store (optional)<input name="store" maxLength={120} /></label><label>Quantity<input name="quantity" maxLength={40} /></label><label>Category or aisle<input name="category" maxLength={80} /></label><label>Priority<select name="priority" defaultValue="normal"><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option></select></label></>;
  if (slug === "pets") return <><label>Species<input name="species" required maxLength={80} /></label><label>Breed (optional)<input name="breed" maxLength={100} /></label><label>Birthdate<input name="birthDate" type="date" /></label></>;
  if (slug === "contacts") return <><label>Phone<input name="phone" type="tel" /></label><label>Email<input name="email" type="email" /></label></>;
...
export function DomainRoom({ slug, records, canManage, shoppingType }: { slug: DomainSlug; records: DomainRecord[]; canManage: boolean; shoppingType?: "grocery" | "household" }) {
  ...
  return (
    <div className={styles.room}>
      {canCreate ? (
        <Card>
          ...
          <form action={createDomainRecord.bind(null, slug)} className={styles.form}>
            ...
            <ExtraFields slug={slug} shoppingType={shoppingType} />
      ...
      {slug === "shopping" && records.some((record) => record.status === "purchased") ? <details><summary>Clear completed items</summary><p>This permanently removes purchased items from these lists.</p><form action={clearCompletedShoppingItems.bind(null, "/shopping")}><button className={styles.secondary} type="submit">Confirm clear completed</button></form></details> : null}
          ...
          {records.map((record) => (
            <li key={record.id}>
              <Card className={styles.record}>
                ...
                {slug === "shopping" ? (
                  <form action={toggleShoppingItem.bind(null, record.id, record.status !== "purchased", "/shopping")}>
                ...
                {(canManage || slug === "shopping") ? (
                  <details>
                    ...
                    <form action={updateDomainRecord.bind(null, slug, record.id)} className={styles.editForm}>
                      <input type="hidden" name="listType" value={shoppingType ?? "household"}/><label>Title<input name="title" defaultValue={record.title} required maxLength={160} /></label>
                      <label>Notes<textarea name="notes" defaultValue={record.notes} rows={2} maxLength={2000} /></label>
                      <button type="submit">Save changes</button>
                      {canManage ? <button className={styles.danger} formAction={removeDomainRecord.bind(null, slug, record.id, slug === "shopping" ? "/shopping" : undefined)} type="submit">Remove</button> : null}
                    </form>
                  </details>
                ) : null}
              </Card>
