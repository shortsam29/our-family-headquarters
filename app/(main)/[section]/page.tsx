...
export default async function SecondaryRoomPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{status?:string;error?:string}> }) {
  ...
  const [data,related,status] = await Promise.all([getDomainRoomData(context,section,section === "shopping" ? "household" : undefined),getRelatedDomainData(context,section),searchParams]);
  ...
  return <FeaturePage>
    ...
    <FeatureSection title={`Manage ${destination.title.toLowerCase()}`} description="Add and update household information directly here. Kenzie can help you plan, but is never required."><DomainRoom slug={section} records={data.records} canManage={canManage} shoppingType={section === "shopping" ? "household" : undefined}/></FeatureSection>
