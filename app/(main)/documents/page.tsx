...
export default async function DocumentsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; error?: string }> }) {
  ...
  return <FeaturePage>
    ...
    <FeatureSection title="Find a document">
      <form action="/documents" role="search"><label htmlFor="vault-search">Search titles, categories, and notes</label><input id="vault-search" name="q" defaultValue={params.q} /><button type="submit">Search</button></form>
    </FeatureSection>
